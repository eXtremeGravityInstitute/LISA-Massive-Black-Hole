...
void fullphaseamp(struct Data *dat, int ll, int K, double *params, double *freq, double *Aamp, *Eamp, *Aphase, *Ephase)
{
    ...
    // reference amplitude and phase
    Intrinsic(ll, params, dat->Tstart, K, freq, TF, PF, AF);
    ...
    */
    ...
    //fullphaseamp(dat, ll, M, params, het->freq, het->amp[0], het->amp[1], het->phase[0], het->phase[1]);
       ...
       for(id = 0; id < Nch; id++)
           ...
           {
               n = het->fgrid[i];
               //printf("%d %e %e %e\n", i, FF[n], het->freq[i], FF[n]-het->freq[i]);
...
double log_likelihood_het(struct Data *dat, struct Het *het, int ll, double *params)
{
    double **amp, **phase;
    double **hc, **hs;
    double HH, HR, logL;
...
double Fstat_het(struct Data *dat, struct Het *het, int ll, double *params, double tm)
{
    double ***amp, ***phase;
    double **Fparams;
    double HH, HR, logL;
    ...
    for(ii = 0; ii < 4; ii++)
     {
        ...
        for(id = 0; id < Nch; id++)
        {
            ...
            for(i = 0; i < NR; i++)
            {
                ...
                for(j = 0; j <= J; j++) NV[ii] += uu[j]*het->ldc[i][id][j];
                
                if(i < NR-2) NV[ii] -= het->dc[id][i+1]*vv[J];  // correction for overcount
                    ...
                    for(j = 0; j <= J; j++)
                           ...
                           for(j = 0; j <= J; j++) NV[ii] += uu[j]*het->lds[i][id][j];
                           
                           if(i < NR-2) NV[ii] -= het->ds[id][i+1]*vv[J];  // correction for overcount
               ...
               for(id = 0; id < Nch; id++)
                {
                   for(i = 0; i < NR; i++)
                   {
                       ...
                       for(j = 0; j <= J; j++)
                        ...
                        for(j = 0; j <= J; j++) MM[ii][jj] += uu[j]*het->SL[i][id][j];;
                        if(i < NR-2) MM[ii][jj] -= het->aa[id][i+1]*vv[J];  // correction for overcount
...
double SNRFast(struct Data *dat, int ll, double *params)
{
    ...
    Intrinsic(ll, params, dat->Tstart, NF, FF, TF, PF, AF);
...
void FisherFast(struct Data *dat, int ll, double *params, double **Fisher)
{
    ...
    Intrinsic(ll, params, Tstart, NF, FF, TFref, PFref, AFref);
    ...
    for (i=0; i< 4; i++)
    {
        ...
        else
        {
            ...
            Intrinsic(ll,paramsP, Tstart, NF, FF, TF, PF, AF);  // intrinsic
            Extrinsic(paramsP, Tstart, Tend, NF, FF, TF, PF, AF, AampP, EampP, AphaseP, EphaseP, &kxm); // extrinsic
                
            Intrinsic(ll,paramsM, Tstart, NF, FF, TF, PF, AF); // intrinsic
...
void Intrinsic(int ll, double *params, double Tstart, int NF, double *FF, double *TF, double *PF, double *AF)
{
