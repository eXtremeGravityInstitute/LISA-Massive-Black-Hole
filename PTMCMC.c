...
int main(int argc,char **argv)
{
  ...
  FILE* out;
    ...
    for(k=0; k < NS; k++)
    {
        ...
        if(k == rep)
        {
            if(premove[5] < dat->Tstart || premove[5] > dat->Tend) printf("WARMING: source does not merge during the chosen time interval\n");
        }
    ...
    */
    ...
    for (j=0; j< NP; j++) pmax[j] = paramx[who[0]][j];
     ...
     // Hot chain should have an effective SNR of ~5
      ...
      x = logLx[who[0]];
      if(nflag == 1) // have to remove -(d|d)/2 term that is used in noise modeling to get SNR^2/2 piece
        {
             for (i=0; i< dat->Nch; i++)
             {
                 x += 0.5*het->DD[i]/sx[who[0]][i];
                 x += (double)(het->MM-het->MN)*log(sx[who[0]][i]);
             }
        }
     SNR = sqrt(2.0*x);
