...
//gcc -o source_frame source_frame.c -lgsl
...
int main(int argc,char **argv)
{
    ...
    content = double_vector(50);
    
    if(argc<4)
      {
          printf("./source_frame file_in file_out #samples\n");
          return 0;
      }
    ...
    fscanf(in,"%d%lf%lf%lf", &k, &content[0], &m1, &m2);
    for(i = 2; i < NP; i++)
     {
        if(i == 6)
         {
         ...
         {
          fscanf(in, "%lf", &content[i]);
         }
     }
    fscanf(in,"%d%lf%lf%lf%lf", &j, &x, &x, &x, &x);
         
         z = z_DL(DL*1000.0);
         
         fprintf(out,"%d %.12e %.12e %.12e ", k, content[0], m1/(1.0+z), m2/(1.0+z));
         for(i = 2; i < NP; i++)
         {
            if(i == 6)
             {
